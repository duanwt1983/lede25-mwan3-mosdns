...
function ensureCss() {
	if (!document.getElementById('bplus-status-css')) {
		document.head.appendChild(E('link', {
			...
			'rel': 'stylesheet',
			'type': 'text/css',
			'href': L.resource('bandix_plus/status.css', '?v=52')
		}));
	}
	ensureLayoutCss();
	ensureSettingsPortalCss();
}

function ensureSettingsPortalCss() {
	if (document.getElementById('bplus-settings-portal-css'))
		return;
	document.head.appendChild(E('style', { id: 'bplus-settings-portal-css' }, [
		'.topo-bplus-modal-host{position:relative;z-index:1;min-height:0;}',
		'.bplus-page.bplus-page--settings-portal{display:none;position:fixed;inset:0;z-index:10040;pointer-events:none;',
		'background:transparent !important;',
		'--bplus-bg:#FFF;--bplus-card:#f8fafc;--bplus-text:#1f2937;--bplus-sub:#6b7280;',
		'--bplus-border:#e5e7eb;--bplus-accent:#0ea5e9;--bplus-up:#f97316;--bplus-down:#06b6d4;',
		'color:var(--bplus-text);font-size:16px;}',
		'.bplus-page.bplus-page--settings-portal.bplus-portal-active{display:block;background:transparent !important;}',
		'.bplus-page.bplus-page--settings-portal.theme-dark{',
		'--bplus-bg:#131720;--bplus-card:#1d2431;--bplus-text:#e5e7eb;--bplus-sub:#b4c0ce;',
		'--bplus-border:#334155;--bplus-accent:#38bdf8;}',
		'.bplus-page--settings-portal .bplus-modal-overlay{display:none;pointer-events:none;}',
		'.bplus-page--settings-portal .bplus-modal-overlay.show{display:flex;align-items:center;justify-content:center;',
		'pointer-events:auto;background:rgba(15,23,42,.55);}',
		'.bplus-page--settings-portal .bplus-confirm-overlay.show{display:flex;align-items:center;justify-content:center;',
		'pointer-events:auto;background:rgba(15,23,42,.55);}'
	].join('')));
}

function settingsPortalOpen(inst) {
	if (!inst || !inst.el || !inst.el.settingsModalPortal)
		return false;
	var el = inst.el;
	return !!(
		(el.scheduleHubOverlay && el.scheduleHubOverlay.classList.contains('show')) ||
		(el.scheduleRuleOverlay && el.scheduleRuleOverlay.classList.contains('show')) ||
		(el.scheduleDeleteConfirmOverlay && el.scheduleDeleteConfirmOverlay.classList.contains('show')) ||
		(el.deviceDeleteConfirmOverlay && el.deviceDeleteConfirmOverlay.classList.contains('show'))
	);
}
...
var BandixView = view.extend({
	load: function () {
		if (window.__BPLUS_TOPO_SETTINGS)
	...
	notifyError: function (msg, err) {
		if (err) {
			var em = err.message ? String(err.message) : String(err);
			if (/aborted/i.test(em) || err.name === 'AbortError' || err === '0')
				return;
		}
		var txt = msg;
	...
	updateDevicesTableMetricsOnly: function () {
		...
		return Promise.all([
			...
			if (this._settingsOnRefresh)
				this._settingsOnRefresh();
			return Promise.resolve();
			}, this)).catch(L.bind(function (e) {
				if (reqSeq !== this.liveReqSeq) return;
	...
	refreshRateData: function (showErr) {
		return Promise.all([
			...
			if (!this._settingsOnlyMode && !this.isScheduleHubUiOpen()) {
				this.renderIfaceLimitTable();
				this.renderGuestControlTable();
			}
		}, this)).catch(L.bind(function (e) {
	...
	renderDevicesTable: function () {
		if (!this.el || !this.el.deviceHead || !this.el.deviceBody) {
			if (this._settingsOnRefresh)
				this._settingsOnRefresh();
			return;
		}
	...
	renderIfaceLimitTable: function () {
		if (!this.el || !this.el.ifaceLimitBody)
			return;
		var self = this;
	...
	renderGuestControlTable: function () {
		if (!this.el || !this.el.guestRuleBody)
			return;
		var self = this;
	...
	syncSettingsPortalVisibility: function () {
		if (!this._settingsOnlyMode || !this.el || !this.el.settingsModalPortal)
			return;
		var open = settingsPortalOpen(this);
		this.el.settingsModalPortal.classList.toggle('bplus-portal-active', open);
		window.__BPLUS_TOPO_SETTINGS = !!open;
	},

	showScheduleHubModal: function () {
		if (this.el.scheduleHubOverlay)
			this.el.scheduleHubOverlay.classList.add('show');
		this.syncSettingsPortalVisibility();
	},
	...
	bindEmbedDeviceEvents: function () {
		...
		bind(this.el.devicesPagePrev, 'click', L.bind(function (ev) {
			ev.preventDefault();
		...
		bind(this.el.devicesPageNext, 'click', L.bind(function (ev) {
			ev.preventDefault();
			this.devicesPage++;
			this.renderDevicesTable();
		}, this));
		bind(this.el.deviceModeSelect, 'change', L.bind(function () {
		...
		bind(this.el.deviceBody, 'click', L.bind(function (ev) {
		...
		bind(this.el.scheduleForm, 'submit', L.bind(this.submitSchedule, this));
		bind(this.el.schCancel, 'click', L.bind(function (ev) {
			ev.preventDefault();
			this.hideScheduleRuleModal();
		}, this));
		bind(this.el.scheduleRuleDismiss, 'click', L.bind(function (ev) {
			ev.preventDefault();
			this.hideScheduleRuleModal();
		}, this));
		bind(this.el.scheduleHubCloseBtn, 'click', L.bind(function (ev) {
			ev.preventDefault();
			this.closeScheduleHubAll();
		}, this));
		bind(this.el.scheduleDeleteConfirmCancel, 'click', L.bind(function (ev) {
			ev.preventDefault();
			this.hideScheduleDeleteConfirm();
		}, this));
		bind(this.el.scheduleDeleteConfirmOk, 'click', L.bind(function (ev) {
			ev.preventDefault();
			this.runDeleteScheduleConfirmed();
		}, this));
		bind(this.el.deviceDeleteConfirmCancel, 'click', L.bind(function (ev) {
			ev.preventDefault();
			this.hideDeviceDeleteConfirm(false);
		}, this));
		bind(this.el.deviceDeleteConfirmOk, 'click', L.bind(function (ev) {
			ev.preventDefault();
			this.runDeleteDeviceConfirmed();
		}, this));
		bind(this.el.deviceDeleteConfirmOverlay, 'click', L.bind(function (ev) {
			if (ev.target === this.el.deviceDeleteConfirmOverlay)
				this.hideDeviceDeleteConfirm(false);
		}, this));
	},

	bindScheduleModalEvents: function () {
		if (this._scheduleModalEventsBound)
			return;
		this._scheduleModalEventsBound = true;
		var bind = function (el, ev, fn) {
			if (el) el.addEventListener(ev, fn);
		};
		bind(this.el.scheduleForm, 'submit', L.bind(this.submitSchedule, this));
	...
	_buildScheduleModalsPartial: function () {
		if (this.el && this.el.scheduleHubOverlay)
			return;
		if (!this.el)
			this.el = {};
	...
	_attachSettingsModals: function (host) {
		host = host || document.getElementById('topo-bplus-modal-host') || document.body;
		if (!this.el.settingsModalPortal) {
			var theme = getThemeMode();
			this.el.settingsModalPortal = E('div', {
				'class': 'bplus-page bplus-page--settings-portal theme-' + theme,
				'id': 'bplus-topo-settings-portal'
			});
		} else {
			this.el.settingsModalPortal.className =
				'bplus-page bplus-page--settings-portal theme-' + getThemeMode();
		}
		var portal = this.el.settingsModalPortal;
		var modals = [
	...
	ensureSettingsModals: function (host) {
		ensureCss();
		if (!this.el)
			this.el = {};
		if (!this.el.scheduleHubOverlay)
			this._buildScheduleModalsPartial();
		var rum = this.rateUnitMode || (localStorage.getItem('bplus_rate_unit') === 'bit' ? 'bit' : 'byte');
		this.rateUnitMode = rum;
		setRateUnitMode(rum);
		this.syncRateLimitUnitControls(rum);
		this._attachSettingsModals(host || document.body);
	},

	prepareSettingsOnly: function (host) {
		this._settingsOnlyMode = true;
		this.ensureSettingsModals(host || document.body);
		if (!this._scheduleModalEventsBound) {
			this.bindScheduleModalEvents();
			this._scheduleModalEventsBound = true;
		}
	},
	...
	buildView: function () {
		...
		this.el.mainSection = E('div', { 'class': 'bplus-main' }, [
				...
				this.el.deviceListSection = E('section', { 'class': 'bplus-panel' }, [
					E('div', { 'class': 'bplus-panel-head' }, [
						E('h2', {}, [ _('Device List') ]),
	...
	ensureEmbedModals: function (root) {
		if (this._settingsOnlyMode)
			return this.ensureSettingsModals(root);
		if (!this.el) this.el = {};
		var host = root || document.body;
		if (!this.el.deviceDeleteConfirmOverlay) {
			var keepKeys = [
				'devicesIfaceSelect', 'periodSelect', 'deviceModeSelect', 'devicesCount',
				'devicesSearchInput', 'devicesLimitSelect', 'devicesPageSizeSelect',
				'devicesPagePrev', 'devicesPageNext', 'devicesPageNum', 'devicesPageInfo',
				'devicesPagination', 'deviceHead', 'deviceBody', 'deviceListSection'
			];
			var kept = {};
			for (var ki = 0; ki < keepKeys.length; ki++) {
				var k = keepKeys[ki];
				if (this.el[k]) kept[k] = this.el[k];
			}
			this.buildView();
			this.setRateUnitMode(this.rateUnitMode, false);
			for (var kj = 0; kj < keepKeys.length; kj++) {
				var kk = keepKeys[kj];
				if (kept[kk]) this.el[kk] = kept[kk];
			}
		}
		var modals = [
			this.el.scheduleHubOverlay,
	...
	mountDeviceListEmbed: function (container, opts) {
		...
		this.syncRateLimitUnitControls(this.rateUnitMode);
	...
	render: function (load) {
		if (window.__BPLUS_TOPO_SETTINGS)
			return E('div', { 'class': 'bplus-render-suppressed', 'style': 'display:none' });
		var viewRoot = document.getElementById('view') || document.getElementById('maincontent');
		var topoPage = viewRoot && viewRoot.querySelector('.topo-page');
		var topoPane = viewRoot && viewRoot.querySelector('#lede-topo-pane');
		if (topoPage || topoPane)
			return E('div', { 'class': 'bplus-render-suppressed', 'style': 'display:none' });
		ensureCss();
		this.initState(load);
...
var __bplusSettingsCtrl = null;

function createBandixCtrl(BandixView) {
	var proto = BandixView && BandixView.prototype;
	if (!proto && BandixView && typeof BandixView === 'object')
		proto = BandixView;
	if (!proto)
		throw new Error('invalid Bandix view module');
	return Object.create(proto);
}

BandixView.createDeviceListEmbed = function (container, opts) {
	var inst = createBandixCtrl(BandixView);
	inst.initState([]);
	return inst.mountDeviceListEmbed(container, opts || {});
};

BandixView.openDeviceSettingsModal = function (dev, ctx) {
	ctx = ctx || {};
	if (!dev)
		return Promise.reject(new Error('missing device'));
	var host = ctx.host || document.getElementById('topo-bplus-modal-host') || document.body;
	if (!__bplusSettingsCtrl) {
		__bplusSettingsCtrl = createBandixCtrl(BandixView);
		__bplusSettingsCtrl.initState([]);
	}
	var inst = __bplusSettingsCtrl;
	inst.prepareSettingsOnly(host);
	inst._settingsOnRefresh = typeof ctx.onRefresh === 'function' ? ctx.onRefresh : null;
	inst.devices = ctx.devices || inst.devices || [];
	if (!inst.rate)
		inst.rate = { schedules: [], ifaceLimits: [], guestDefaults: [], guestWhitelist: [] };
	if (ctx.schedules && ctx.schedules.length)
		inst.rate.schedules = ctx.schedules;
	return inst.refreshRateData(false).then(function () {
		inst.openScheduleModalForDevice(dev);
		if (typeof inst.renderScheduleHubRulesList === 'function')
			inst.renderScheduleHubRulesList();
		return inst;
	}).catch(function (e) {
		var m = e && e.message ? String(e.message) : String(e || '');
		if (/aborted/i.test(m) || e === '0' || (e && e.name === 'AbortError')) {
			inst.openScheduleModalForDevice(dev);
			return inst;
		}
		throw e;
