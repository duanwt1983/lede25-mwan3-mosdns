...
$local = Join-Path $repoRoot 'files\www\luci-static\argon\css\lede-brand-font.css'
$remote = '/www/luci-static/argon/css/lede-brand-font.css'
...
function Write-RemoteFile($localPath, $remotePath) {
    ...
    Invoke-Exec ("mkdir -p '/www/luci-static/argon/css'") | Out-Null
    return Invoke-Exec ("base64 -d '$tmp' > '$remotePath' && rm -f '$tmp' && grep font-size '$remotePath'")
}
...
Invoke-Exec ("chmod 644 '$remote'") | Out-Null
Write-Output 'CSS_HOTFIX_DONE'
